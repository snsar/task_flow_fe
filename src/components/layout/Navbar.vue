...
const userAvatar = computed(() => authStore.user?.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(userName.value))
  ...
  <div class="navbar bg-base-100 shadow-md">
    ...
    <div class="flex-none gap-2">
      <div v-if="authStore.isAuthenticated" class="dropdown dropdown-end">
        ...
        <div v-if="showNotifications" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-80 mt-4">
          <div class="flex justify-between items-center p-2 border-b">
            <h3 class="font-bold">Thông báo</h3>
            <button @click="markAllAsRead" class="btn btn-xs btn-ghost">Đánh dấu tất cả đã đọc</button>
          </div>
          ...
          <ul v-else class="max-h-96 overflow-y-auto">
            <li v-for="notification in notificationStore.notifications" :key="notification.id" 
              ...
              <div class="flex justify-between">
                <div>
                  <p class="text-sm" :class="{ 'font-bold': !notification.read }">{{ notification.message }}</p>
                  <p class="text-xs text-gray-500">{{ new Date(notification.created_at).toLocaleString() }}</p>
                </div>
                <div class="flex gap-1">
                  <button v-if="!notification.read" @click="markAsRead(notification.id)" class="btn btn-xs btn-ghost">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                  </button>
                  <button @click="deleteNotification(notification.id)" class="btn btn-xs btn-ghost text-error">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div v-if="authStore.isAuthenticated" class="dropdown dropdown-end">
        ...
        <ul v-if="showUserMenu" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 mt-4">
