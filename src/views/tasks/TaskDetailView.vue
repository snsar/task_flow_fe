...
const openEditTaskModal = () => {
  editedTask.value = {
    ...
    assigned_to: taskStore.currentTask.assigned_to
  }
...
const goToProject = () => {
  if (taskStore.currentTask?.project_id) {
    router.push(`/projects/${taskStore.currentTask.project_id}`)
  }
}
  ...
  <AppLayout>
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <p class="text-gray-500 mt-1">
            Dự án: <a @click="goToProject" class="link link-primary cursor-pointer">{{ taskStore.currentTask.project?.name }}</a>
          </p>
        </div>
        ...
        <div class="flex gap-2 mt-4 md:mt-0">
          <div v-if="canEdit" class="dropdown dropdown-end">
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
              <li><a @click="updateTaskStatus('todo')" :class="{ 'bg-base-200': taskStore.currentTask.status === 'todo' }">Cần làm</a></li>
              <li><a @click="updateTaskStatus('in_progress')" :class="{ 'bg-base-200': taskStore.currentTask.status === 'in_progress' }">Đang làm</a></li>
              <li><a @click="updateTaskStatus('completed')" :class="{ 'bg-base-200': taskStore.currentTask.status === 'completed' }">Hoàn thành</a></li>
            </ul>
          </div>
          ...
          <div v-if="canEdit || canDelete" class="dropdown dropdown-end">
            ...
            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
              ...
              <li v-if="canDelete"><a @click="openDeleteConfirmModal" class="text-error">Xóa công việc</a></li>
            </ul>
          </div>
        </div>
      </div>
                  ...
                  <div class="flex items-center mt-1">
                    <div class="avatar mr-2">
                      <div class="w-6 h-6 rounded-full">
                        <img 
                          :src="taskStore.currentTask.creator?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(taskStore.currentTask.creator?.name || 'User')}`" 
                ...
                <div>
                  ...
                  <div v-if="taskStore.currentTask.assignedUser" class="flex items-center mt-1">
                    <div class="avatar mr-2">
                      <div class="w-6 h-6 rounded-full">
                        <img 
                          :src="taskStore.currentTask.assignedUser.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(taskStore.currentTask.assignedUser.name)}`" 
    ...
    <dialog :open="showDeleteConfirmModal" class="modal modal-bottom sm:modal-middle">
      <div class="modal-box">
        ...
        <p class="py-4">Bạn có chắc chắn muốn xóa công việc này? Hành động này không thể hoàn tác.</p>
